...
;Non-overlapping with string functions
...
mov rdi,array+40	;address of 6th element

;the only difference between Non-overlapping without string functions and Non-overlapping with string functions lis in following lines
mov rcx,5	;rcx is default counter register
cld		;cld is instruction which stands for clear direction flag.It clears the direction flag so that rsi and rdi are incremented
rep movsq	;movsq is string instruction used to move qword from source(i.e rsi) to destination(i.e rdi) the instruction is repeated until counter is zero
...
hToA:			;this block converts hex to ASCII
